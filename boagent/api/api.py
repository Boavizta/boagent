...
async def csv(
    data: str, since: str = "now", until: str = "24h", inwatt: bool = True
) -> Response:
    ...
    return Response(
        status_code=501, content="Converting data to CSV is not implemented yet."
    )
...
async def last_data(table_name: str) -> Response:
    ...
    status_code=200)"""
    return Response(
        status_code=501, content="Getting last data is not implemented yet."
    )
...
async def last_info():
    res = {
        "power": get_most_recent_data("power"),
        "carbonintensity": get_most_recent_data("carbonintensity"),
        "cpu": get_most_recent_data("cpu"),
        "ram": get_most_recent_data("ram"),
    }
...
async def max_info():
    res = {
        "power": get_max("power"),
        "carbonintensity": get_max("carbonintensity"),
        "ram": get_max("ram"),
        "cpu": get_max("cpu"),
    }
    return res
...
async def update():
    ...
    session.commit()
    session.close()"""
    return Response(status_code=501, content="Update route is not implemented yet.")
...
async def carbon_intensity(since: str = "now", until: str = "24h") -> Response:
    ...
    return Response(
        status_code=501, content="Getting carbon intensity is not implemented yet."
    )
...
async def init_carbon_intensity():
    ...
    session.commit()"""
    return Response(
        status_code=501, content="Init Carbon intensity is not implemented yet."
    )
...
async def impact(since: str = "now", until: str = "24h") -> Response:
    ...
    return Response(status_code=501, content="Getting impact is not implemented yet.")
...
def query_electricity_carbon_intensity(
    start_date: Optional[datetime] = None, stop_date: Optional[datetime] = None
) -> Dict[str, Any]:
    """DEPRECATED BOAVIZTAPI ROUTE"""
    url = (
        BOAVIZTAPI_ENDPOINT
        + f"/v1/usage_router/gwp/current_intensity?location={AZURE_LOCATION}"
    )
...
def parse_electricity_carbon_intensity(carbon_aware_api_response: Dict[str, Any]):
    ...
    if "endTime" in intensity_dict and "carbonIntensity" in intensity_dict:
        ...
        return {"timestamp": datetime.now(), "value": 0}


def query_forecast_electricity_carbon_intensity(
    start_date: datetime, stop_date: datetime
) -> Dict[str, Any]:
    """DEPRECATED BOAVIZTAPI ROUTE"""
    url = (
        BOAVIZTAPI_ENDPOINT
        + f"/v1/usage_router/gwp/forecast_intensity?location={AZURE_LOCATION}"
    )
    retry = 0
    while retry < 3:
        retry += 1
                ...
                if "dataStartAt" in errors:
                    if len(errors["dataStartAt"]) == 1:
                        ...
                        start_date = datetime.strptime(
                            error_msg[1][:-10], "%m/%d/%Y %H:%M:%S"
                        )
                        stop_date = datetime.strptime(
                            error_msg[3][:-10], "%m/%d/%Y %H:%M:%S"
                        )
...
def parse_forecast_electricity_carbon_intensity(
...
def parse_date_info(
    since: str, until: str, forecast: bool = False
...
def compute_recommendations(since="now", until="24h"):
    ...
    df_carbon_intensity["timestamp"] = df_carbon_intensity["timestamp"].apply(
        utc.localize
    )
            ...
            recommendations.append(
                {
                    ...
                    "preferred_execution_date": find_preferred_execution_date_in_future(
                        df_forecast
                    ),
                    "mode": "forecast",
                    "job": cron["job"],
            ...
            recommendations.append(
                {
                    ...
                    "execution_date": datetime.strptime(
                        str(cron["previous"])[:-6], "%Y-%m-%d %H:%M:%S"
                    ),
                    "preferred_execution_date": find_preferred_execution_date_in_history(
                        cron["previous"], df_power, df_history
                    ),
                    "mode": "history",
                    "job": cron["job"],
                }
...
def find_preferred_execution_date_in_history(
    execution_date: datetime, df_power: pd.DataFrame, df_intensity: pd.DataFrame
) -> datetime:
    ...
    for row in bests.itertuples():
        new_execution_date = utc.localize(
